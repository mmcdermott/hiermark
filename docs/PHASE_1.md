# Phase 1 — @hiermark/editor surface MVP
...
**Editor engine** — `createHiermarkEditorExtensions()` assembles StarterKit, TaskList/
TaskItem, Placeholder, the official `@tiptap/markdown` (so `editor.getMarkdown()`
and markdown content load losslessly), KaTeX math, and the Hiermark block-id
...
**Tree-shaped snapshots** — `getHiermarkSurfaceSnapshot(editor, …)` walks the live
ProseMirror tree (never markdown) into a `HiermarkSurfaceSnapshot` with `parentId`/
...
a `getContext` getter. `HiermarkEditor` turns a click into an `onBranchRequest` event
...
hashing, the `hiermark:` stable-id comment grammar, heading containment, fenced-code-
...
**`HiermarkEditor` component** — local (non-collab) editor wiring all of the above,
emitting `onChange`/`onSnapshotChange`/`onBranchRequest`/`onActiveBlockChange`
and publishing a typed `HiermarkEditorHandle` (focus/scroll/snapshot/markdown/json/
...
- `HiermarkEditor`: tree snapshot from a real editor, markdown round-trip, branch
...
```text
pnpm build        ✓  @hiermark/editor → dist (44 exports) + styles.css
pnpm typecheck    ✓
pnpm lint         ✓
...
- **Collaboration** (Phase 3) — `HiermarkEditor` is structured so the collab mount/
...
- Phase 2: `@hiermark/canvas` — surface/edge model, `projectHiermarkColumns`,
  `getHiermarkActivePath`, branch/sibling flows, dnd reorder.
