...
A plain-language tour of what the product is *for*, written as design input for
...
tree out as *levels left→right, sections top→down*, so you can read the whole
...
*Leans on:* the pure tree model + topology ops (`doc-tree`), the levels/grid
...
and the HSM are the *same* machinery with different presentation (no "Level N"
labels, roomier cards, any-block branching vs. root-only).

*Leans on:* block-anchored branching + provenance edges, the shared grid/card with
...
document, the system records *where it came from* — a `branches_from` edge anchored
...
*Leans on:* the anchored-graph data model (documents + edges with block/span
...
everywhere; deleting it from one document does *not* delete it while another
document still has it. A tasks panel aggregates them.

*Leans on:* the markdown reconciler (checklist → task, by stable key) and the
...
*Leans on:* the editor's entity-decoration framework (recognizer × placement),
...
*Leans on:* the markdown engine (canonical grammar + projection) and the editor's
...
*Leans on:* the editor's collaboration adapter (today Hocuspocus/Yjs, behind an
...
*Leans on:* block containment (heading-stack sectioning) + the editor fold
extension.
...
*Leans on:* the app shell + lab/project/submission scoping (application layer, not
a reusable package).
...
- **git-sync round-trip** — export the document tree to plain markdown files,
  ...
  identity. This is the forcing function for making stable IDs *markdown-portable*
...
1. **Pure document topology** — the tree/graph types and immutable ops, with edge
   identity and anchor metadata preserved (scenarios 1–3). *No React, no DB.*
2. **The canonical markdown engine** — projection, reconciliation, stable-id
   grammar, checklist/citation/resource extraction (scenarios 4–6). *Pure.*
